...
fn not_implemented() -> impl IntoResponse {
    (StatusCode::NOT_IMPLEMENTED, Json(serde_json::json!({
        "error": "graph store not yet collection-scoped"
    })))
}
...
pub async fn graph_list(headers: HeaderMap, State(engine): State<Option<Arc<ArcanumEngine>>>) -> impl IntoResponse {
    if let Err(e) = validate_bearer(&headers, &engine) { return e.into_response(); }
    not_implemented().into_response()
}
pub async fn graph_create(headers: HeaderMap, State(engine): State<Option<Arc<ArcanumEngine>>>, Path(_): Path<String>) -> impl IntoResponse {
    if let Err(e) = validate_bearer(&headers, &engine) { return e.into_response(); }
    not_implemented().into_response()
}
pub async fn graph_delete(headers: HeaderMap, State(engine): State<Option<Arc<ArcanumEngine>>>, Path(_): Path<String>) -> impl IntoResponse {
    if let Err(e) = validate_bearer(&headers, &engine) { return e.into_response(); }
    not_implemented().into_response()
}
pub async fn graph_stats_all(headers: HeaderMap, State(engine): State<Option<Arc<ArcanumEngine>>>) -> impl IntoResponse {
    if let Err(e) = validate_bearer(&headers, &engine) { return e.into_response(); }
    not_implemented().into_response()
}
pub async fn graph_stats_one(headers: HeaderMap, State(engine): State<Option<Arc<ArcanumEngine>>>, Path(_): Path<String>) -> impl IntoResponse {
    if let Err(e) = validate_bearer(&headers, &engine) { return e.into_response(); }
    not_implemented().into_response()
}
...
mod tests {
    ...
    #[tokio::test]
    async fn graph_collections_return_501() {
        let app = build_app(None);
        // No engine → 401, not 501, because auth check happens first.
        // With a real engine and no graph store configured, 501 is returned.
        // This test verifies the route is registered (not 404).
        let resp = app.oneshot(
            Request::builder().uri("/api/v1/graph/collections").body(Body::empty()).unwrap()
        ).await.unwrap();
