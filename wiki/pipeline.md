...
TextEnricher>>` (reusing `deps.context_enricher`); both are
...
unchanged. It then calls the free function `run_task`.
...
1. Any stage's `run` returning `Err` short-circuits `DagExecutor::execute`
   and is logged and returned; earlier waves' side effects (a `cleanup`
...
- **`Bm25Index` has no delete-by-`source_uri`, so `make_cleanup_stage`
  ...
  replace, but `Bm25Index` only supports single-chunk delete-by-id, so a
