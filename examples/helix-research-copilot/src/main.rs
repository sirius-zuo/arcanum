...
//!
//! Research intelligence — FULL ingestion (entity graph + RAPTOR tree + contextual
//! enrichment), QueryClassified retrieval. The classifier routes entity queries to
//! Graph, synthesis queries to RAPTOR, and broad queries to Vector + BM25.
//!
//! Dev uses in-memory graph + tree stores and an Ollama enricher — no external
//! services required. See BUILD.md for the production stores (Neo4j, Postgres, GLiNER,
//! Claude).
//!
//! Run:
...
async fn main() -> Result<()> {
    ...
    println!("┌────────────────────────────────────────────────┐");
    println!("│  Helix Research Copilot                         │");
