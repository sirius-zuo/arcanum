...
| Neo4j 5 | Knowledge graph | `neo4j:5` |
| HuggingFace TEI | Embeddings | `ghcr.io/huggingface/text-embeddings-inference:cpu-1.5` |
| GLiNER service | Entity extraction (no GPU needed) | `urchade/gliner` (self-hosted) |

Claude (Anthropic API) handles ContextPrefix and Summarize. Alternatively keep
Ollama for those two intents.
...
```rust
// REMOVE the four dev stores/enricher:
...
| `NEO4J_URL`, `NEO4J_USER`, `NEO4J_PASSWORD` | Yes | knowledge graph |
